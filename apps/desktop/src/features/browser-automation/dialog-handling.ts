import type { BrowserAutomationDialogEvent } from "@frogg/protocol/browser-automation/rpc-schemas";
...
export function promptShimInstallScript(): string {
  return String.raw`(() => {
    const stateKey = "__FROGG_BROWSER_AUTOMATION_DIALOG_STATE__";
...
export function promptShimDrainScript(): string {
  return String.raw`(() => {
    const state = window.__FROGG_BROWSER_AUTOMATION_DIALOG_STATE__;
