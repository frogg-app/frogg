...
interface BrowserCaptureClipboard<TImage extends BrowserCaptureImage> {
  write(input: { text: string; image: TImage }): void | Promise<void>;
  writeImage(image: TImage): void | Promise<void>;
...
export function createBrowserCaptureService<TImage extends BrowserCaptureImage>(
  ...
  return {
    ...
    async copy(payload) {
      ...
      if (text && image) await dependencies.clipboard.write({ text, image });
      else if (image) await dependencies.clipboard.writeImage(image);
      else if (text) await dependencies.clipboard.writeText(text);
      else return false;
      return true;
    },
  };
