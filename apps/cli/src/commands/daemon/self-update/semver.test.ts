 ...
 *   1.3.5-beta.1 < 1.3.5-beta.2 < 1.3.5 < 1.3.5-gl.1 < 1.3.5-gl.2 < 1.3.6
...
describe("downstream rebuild ordering", () => {
  it("ranks a downstream rebuild above the release it rebuilds", () => {
    expect(isNewerVersion("1.3.5-gl.1", "1.3.5")).toBe(true);
    expect(isNewerVersion("1.3.5", "1.3.5-gl.1")).toBe(false);
  });

  it("ranks later rebuild counters above earlier ones", () => {
    expect(isNewerVersion("1.3.5-gl.2", "1.3.5-gl.1")).toBe(true);
    expect(isNewerVersion("1.3.5-gl.3", "1.3.5-gl.2")).toBe(true);
    expect(isNewerVersion("1.3.5-gl.2", "1.3.5-gl.3")).toBe(false);
    expect(isNewerVersion("1.3.5-gl.10", "1.3.5-gl.9")).toBe(true);
  });

  it("treats an identical version as not newer", () => {
    expect(isNewerVersion("1.3.5-gl.2", "1.3.5-gl.2")).toBe(false);
    expect(isNewerVersion("1.3.5", "1.3.5")).toBe(false);
  });

  it("keeps the next upstream release above every rebuild of the previous one", () => {
    expect(isNewerVersion("1.3.6", "1.3.5-gl.9")).toBe(true);
    expect(isNewerVersion("1.3.5-gl.9", "1.3.6")).toBe(false);
  });
  ...
  it("ranks a rebuild above any prerelease of the same version", () => {
    expect(isNewerVersion("1.3.5-gl.1", "1.3.5-rc.1")).toBe(true);
  });

  it("orders a rebuild of a prerelease by its upstream part first", () => {
    expect(isNewerVersion("1.3.5-beta.1.gl.1", "1.3.5-beta.1")).toBe(true);
    expect(isNewerVersion("1.3.5-beta.2", "1.3.5-beta.1.gl.9")).toBe(true);
    expect(isNewerVersion("1.3.5", "1.3.5-beta.1.gl.9")).toBe(true);
  });

  it("sorts a full ladder ascending", () => {
    const ladder = ["1.3.5-gl.2", "1.3.6", "1.3.5", "1.3.5-beta.1", "1.3.5-gl.1", "1.3.5-beta.2"];
...
describe("parseVersion suffix classification", () => {
  it("splits a downstream rebuild suffix", () => {
    const parsed = parseVersion("v1.3.5-gl.3");
    expect(parsed?.raw).toBe("1.3.5-gl.3");
    expect(parsed?.prerelease).toBe("gl.3");
    expect(parsed?.upstreamPrerelease).toBeNull();
    expect(parsed?.downstreamBuild).toBe("gl.3");
  });
  ...
  it("splits a rebuild of a prerelease into both parts", () => {
    const parsed = parseVersion("1.3.5-beta.1.gl.2");
    expect(parsed?.upstreamPrerelease).toBe("beta.1");
    expect(parsed?.downstreamBuild).toBe("gl.2");
  });
});

describe("artifactVersion", () => {
  it("drops the downstream rebuild counter from artifact filenames", () => {
    expect(artifactVersion("1.3.5-gl.3")).toBe("1.3.5");
    expect(artifactVersion("v1.3.5-gl.3")).toBe("1.3.5");
  });
  ...
  it("keeps a plain version and an upstream prerelease intact", () => {
    ...
    expect(artifactVersion("1.3.5-beta.1.gl.2")).toBe("1.3.5-beta.1");
  });
});
...
describe("isStableVersion", () => {
  it("counts a downstream rebuild of a stable release as stable", () => {
    ...
    expect(isStableVersion("1.3.5-gl.2")).toBe(true);
    expect(isStableVersion("1.3.5-beta.1")).toBe(false);
    expect(isStableVersion("1.3.5-beta.1.gl.1")).toBe(false);
