 ...
 *
 * This fork rebuilds an upstream release under tags such as `v1.3.5-gl.2`: the
 * upstream version is unchanged, and `gl.2` counts *rebuilds of it*. Formally
 * that is a semver prerelease, which would sort `1.3.5-gl.2` BELOW plain
 * `1.3.5` — exactly backwards, and the reason a `gl.1` -> `gl.2` self-update
 * was reported as "already up to date". A downstream rebuild is newer than the
 ...
 * fork's tag works: a prerelease whose first identifier is a well-known
 * *upstream channel* name (`alpha`, `beta`, `rc`, ...) keeps standard semver
 * ordering and sorts below the release; any other suffix is read as a
 * downstream rebuild counter and sorts above it. A rebuild of a prerelease
 * (`1.3.5-beta.1.gl.1`) carries both parts and is ordered by the upstream part
 * first.
 *
...
export interface ParsedVersion {
  ...
  /** The downstream rebuild part of the suffix (`gl.2`), or null when this is an upstream build. */
 ...
 * downstream rebuild counter: `1.3.5-gl.2` ships
