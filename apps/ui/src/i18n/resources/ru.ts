  ...
  agentControls: {
    ...
    account: {
      ...
      transfer: {
        ...
        subtitle: "Выполняется от имени {{name}}. Выберите учётную запись для продолжения.",
        warningTitle: "{{name}} не видел этот разговор",
        warningTitleUnselected: "Выбранная учётная запись не видела этот разговор",
        warningBody:
          "Перенос заново отправит весь контекст объёмом {{tokens}} токенов как новый ввод. Ничто из этого не считается чтением из кэша, и эти входные токены будут оплачены сразу же при отправке следующего сообщения.",
        warningBodyUnknownTokens:
          "Перенос заново отправит весь контекст как новый ввод. Ничто из этого не считается чтением из кэша, и эти входные токены будут оплачены сразу же при отправке следующего сообщения.",
