...
function StreamGraphSvg({
  ...
  const colorOf = useMemo(() => {
    const map = new Map(
      layout.lanes.map((lane) => [lane.stream.id, laneColor(palette, lane.stream.id, lane.stream.channel)]),
    );
    ...
    <ScrollView horizontal showsHorizontalScrollIndicator contentContainerStyle={{ minWidth: width }}>
      <Svg width={width} height={height} accessibilityRole="image" testID="release-streams-graph">
        {layout.lanes.map((lane) => {
          ...
          return (
            <G key={lane.stream.id}>
              <SvgText x={8} y={laneY - 2} fontSize={12} fontWeight="600" fill={palette.text}>
                {lane.stream.label}
              </SvgText>
              <SvgText x={8} y={laneY + 13} fontSize={10} fill={color}>
        ...
        {layout.edges.map((edge) => {
          ...
          const midY = (y(edge.from.lane) + y(edge.to.lane)) / 2;
          return (
            <G key={edge.key}>
              <Path
                ...
                strokeWidth={2}
                fill="none"
                strokeDasharray={edge.kind === "backport" || edge.kind === "contribute" ? "5 3" : undefined}
              />
          ...
          return (
            <G key={`${arrow.flow.kind}:${arrow.flow.from}:${arrow.flow.to}`}>
              <Path
                ...
                strokeDasharray="3 3"
              />
              <Circle cx={x(arrow.to.column) + lift} cy={y(arrow.to.lane)} r={3} fill={palette.pending} />
              <Rect
              ...
              />
              <SvgText x={x(arrow.from.column) + lift + 9} y={midY + 3} fontSize={9} fill={palette.muted}>
                {text}
              </SvgText>
            </G>
          );
          ...
          if (node.head) {
            return (
              <G key={node.key}>
                ...
                />
                <SvgText x={cx} y={cy + 22} fontSize={10} textAnchor="middle" fill={palette.muted}>
