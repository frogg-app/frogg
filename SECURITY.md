...
Stock Frogg binds to `0.0.0.0:9999` and trusts private-network clients (`daemon.auth.trustLan`); other brands default to `127.0.0.1`, with LAN trust off and claim mode on. With no password configured, loopback clients, and LAN clients while LAN trust is on, are trusted by network reachability — anything that can reach the daemon socket from there can control the daemon. This is the same security model Docker documents for its daemon: the security boundary is access to the socket or listening address. Everyone else needs a paired-device credential.

A bearer that is presented is always checked: a revoked, wrong or unknown token is refused with `401` (WebSocket close `4401`) even from loopback. The CLI on the daemon host authenticates with the `local-token` file in the daemon home (mode 0600), which the daemon accepts only from loopback, Unix socket and named pipe clients. Minting a pairing offer or code on a claimed daemon needs a real credential (owner device, password or local token), not locality alone.

In claim mode (`daemon.auth.claimMode`) the LAN is untrusted and the first client to claim an unclaimed daemon becomes its owner. `daemon.auth.claimScope: "local"` restricts that claim to a loopback client presenting the local token. The claim is latched: revoking every device does not reopen it; only `auth reset-claim` on the host does. The daemon refuses to revoke or demote the last owner. Turning claim mode off hands LAN access back to `trustLan`, so set `trustLan: false` with it on networks you do not control. Scripted fleet installs are covered in the [provisioning checklist](https://frogg.app/docs/self-hosting/provisioning/).

The daemon also supports an optional shared-secret password (set via `daemon.auth.password` in `config.json`, stored as a scrypt hash with legacy bcrypt hashes still accepted, or the `FROGG_PASSWORD` env var, `<PREFIX>_PASSWORD` on a branded build). Pass it through the environment, never on a command line. A daemon with a password counts as claimed. When configured, every HTTP request must carry `Authorization: Bearer <password>` and every WebSocket upgrade must include a `Sec-WebSocket-Protocol: frogg.bearer.<password>` subprotocol. Browser WebSocket cannot set custom headers, which is why the token rides in the subprotocol. Health (`GET /api/health`) and CORS preflight (`OPTIONS`) are exempt. The password is intended for direct-TCP exposure (e.g. `tcp://host:port?ssl=true&password=...`); it is **not** a substitute for the relay's E2E encryption when traversing untrusted networks.
